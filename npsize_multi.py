...
def findcube():
    if w > 30 and h/w <1.2 and area*mag**2/(w*h)>0.88:
        ...
        global inset_center, inset_l
        inset_center = (int(rect[0][1]),int(rect[0][0]))
        inset_l = int(max(rect[1]))
        return 1
    else:
        return 0
...
def draw_inset():
    r1 = max(inset_center[0]-inset_l,0)
    r2 = max(inset_center[0]+inset_l,0)
    c1 = max(inset_center[1]-inset_l,0)
    c2 = max(inset_center[1]+inset_l,0)
    im_inset = np.copy(im[r1:r2,c1:c2])
