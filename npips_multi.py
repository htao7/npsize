...
def FindSphere():
    center = (int(x),int(y))
    radius = int(r)
    if w > 20 and area/(3.14*(w/2)**2)>0.88: # standard
    ...
    for i in range(count):
        ...
        for j in range(i + 1,count):
            ...
            if abs(theta1 - theta2) < 5:
                ...
                if dist1 < 30 and  dist1 > 0 and dist2 < -l2/2*mag:
                    ips_temp.append(((npn + i,npn + j),dist1))
                    ipspairs += 1
                    ips += dist1
                    (a,b) = GetPoints(k,theta1,b1,b2,l1,dist1,x1,-y1)
                    cv2.line(im,a,b,(255,0,0),LABEL_THICKNESS)
                    cv2.line(im,a,b,(255,0,0),LABEL_THICKNESS)
                elif dist2 < 30 and dist2 > 0 and dist1 < -l2/2*mag:
                    ...
                    if k == 0:
                        ...
                        b_1 = y1 - k_2*x1
                        b_2 = y2 - k_2*x2
                    (a,b) = GetPoints(k_2,theta1,b_1,b_2,l1,dist2,x1,-y1)
                    cv2.line(im,a,b,(255,0,0),LABEL_THICKNESS)
    return ips,ipspairs
...
def IpsRod():
    ...
    for i in range(count):
        ...
        for j in range(i + 1,count):
            ...
            y2 = -outline_list[j][0][1]
            theta2 = -outline_list[j][2]
            if abs(theta1 - theta2) < 5: # angle difference
                b2 = y2 - k*x2 # kx - y + b2 = 0
                ...
                if l11 < l12 and l21 < l22: # rods pointing to NW
                    ...
                    if dist2 < 50 and  dist2 > 0 and dist1 < -l22/2*mag:
                        ips_temp.append(((npn + i,npn + j),dist2))
                        ipspairs += 1
                        ips += dist2
                        if k == 0:
                            ...
                            b_1 = y1 - k_2*x1
                            b_2 = y2 - k_2*x2
                        (a,b) = GetPoints(k_2,theta1,b_1,b_2,l11,dist2,x1,-y1)
                        cv2.line(im,a,b,(255,0,0),LABEL_THICKNESS)
                if l11 > l12 and l21 > l22: # rods pointing to NE
                    ...
                    dist1 = mag*(d1 - l12/2 - l22/2)
                    dist2 = mag*(d2 - l11/2 - l21/2)
                    if dist1 < 50 and  dist1 > 0 and dist2 < -l21/2*mag:
                        ips_temp.append(((npn + i,npn + j),dist1))
                        ipspairs += 1
                        ips += dist1
                        (a,b) = GetPoints(k,theta1,b1,b2,l12,dist1,x1,-y1)
                        cv2.line(im,a,b,(255,0,0),LABEL_THICKNESS)
                        #draw
    return ips,ipspairs

def GetPoints(k,theta,b1,b2,w,dist,x,y):
# k: slope of long axis; theta: angle
# b1: intercept of i; b2: intercept of j;
# w: width of i; dist: ips
# x,y: center of i
    if k == 'YouF*ckedUp':
        if b1 < b2:
            xa = x + w/2
        ...
        if b1 > b2:
            ...
            xb = x + (w/2 + dist/mag)*math.sin(theta*math.pi/180)
            yb = y + (w/2 + dist/mag)*math.cos(theta*math.pi/180)
        else:
            xa = x - w/2*math.sin(theta*math.pi/180)
            ya = y - w/2*math.cos(theta*math.pi/180)
            xb = x - (w/2 + dist/mag)*math.sin(theta*math.pi/180)
        ...
        if b1 > b2:
            ...
            xb = x - (w/2 + dist/mag)*math.cos(theta*math.pi/180)
            yb = y + (w/2 + dist/mag)*math.sin(theta*math.pi/180)
        else:
            xa = x + w/2*math.cos(theta*math.pi/180)
            ya = y - w/2*math.sin(theta*math.pi/180)
            xb = x + (w/2 + dist/mag)*math.cos(theta*math.pi/180)
